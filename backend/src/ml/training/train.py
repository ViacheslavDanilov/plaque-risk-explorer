...
class AdverseOutcomeModel:
    ...
    def bootstrap_validation(self, data, target_column: str, n_bootstrap: int = 300):
        ...
        final_model = TabularPredictor(label=target_column, verbosity=0)
        final_model.fit(data, hyperparameters={'REALTABPFN-V2.5': {}})

        p_full = final_model.predict_proba(data).iloc[:, 1]
    ...
    def train_loocv(self, data, target_column: str):
        ...
        for i, (train_idx, test_idx) in enumerate(self.loo.split(X)):
            ...
            logger.info(f"Fold {i+1} - True: {true[-1]}, Predicted Probability: {proba:.4f}")
            # self.tabpfnv2_predictor.leaderboard(TabularDataset(test_df), silent=True)
            # self.tabpfnv2_predictor.save('backend/models/tabpfnv2_model_fold_{}'.format(i+1))
            self.evaluate_loocv(true, preds)

            # del self.tabpfnv2_predictor
            gc.collect()
...
if __name__ == "__main__":
    ...
    adverse_outcome_model = AdverseOutcomeModel()
    # adverse_outcome_model.train_stratified(data, target_column="adverse_outcome")
    adverse_outcome_model.bootstrap_validation(data, target_column="adverse_outcome")
