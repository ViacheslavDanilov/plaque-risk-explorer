...
### Использованный режим: `new`

В данном проекте синтетический датасет был сформирован с использованием режима `new`. Это означает, что в итоговый датасет включаются только синтетические наблюдения, созданные алгоритмом oversampling (например, SMOTENC), а все исходные (реальные) данные исключаются. Такой подход особенно полезен для:

- Формирования полностью синтетической популяции для обучения модели, что предотвращает утечку информации и позволяет проводить анализ с сохранением приватности.
- Обеспечения честной оценки модели на действительно невиданных реальных данных, что даёт объективную оценку обобщающей способности.
- Изучения свойств и распределения синтетических данных независимо от исходной выборки.

На практике режим `new` реализован через идентификацию и сохранение только тех наблюдений, которые были сгенерированы семплером и отсутствовали в исходных данных. Итоговый синтетический датасет использовался для обучения, а реальные данные — для тестирования и валидации.
